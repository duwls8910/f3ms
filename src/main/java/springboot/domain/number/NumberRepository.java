...
public interface NumberRepository extends JpaRepository<Number, Long> {
    ...
    Number findByNumber_id(Long number_id);

    //중복검사 jpa exists 찾아보자
    @Query(value = "SELECT COUNT(n.id) > 0 FROM number n WHERE n.number_name =:numberName", nativeQuery = true)
    boolean existsByNumber_name(int numberName);
}
